...
P Read_P_txt(FILE *Entrada){
    P p1;
    fscanf(Entrada, "%d", &p1.Grad);
    for (int i=0; i<=p1.Grad; i++)
    {
        fscanf(Entrada, "%lg", &p1.c[i]);
    }
    while (p1.Grad>=0 && p1.c[p1.Grad]==0)
    {
        p1.Grad--;
    }
    return p1;
...
P Deriva_P(P p1){
    P p_prime;
    p_prime.Grad=p1.Grad-1;
    for (int i=0; i<=p_prime.Grad; i++)
    {
        p_prime.c[i]=p1.c[1+i]*(1+i);
    }
    return p_prime;
}
...
double Newton(P p1, P p2, double Alpha){
    double Value, p1_Val, p2_Val;
    p1_Val=Poli_eval(p1, Alpha);
    p2_Val=Poli_eval(p2, Alpha);
    Value=Alpha-(p1_Val/p2_Val);
    return Value;
}